    ...
    <div className="w-full py-4 space-y-3 md:hidden">
      <div className="flex items-center justify-between px-4">
        <h3 className="text-sm font-bold text-white flex items-center gap-2">
          <Layers className="w-4 h-4 text-indigo-400" style={{ color: colorPreset.primary }} />
          모바일 전용 스와이프 뷰
        </h3>
        <span className="text-[11px] text-slate-400 font-mono">가로로 슬라이드 ➔</span>
      </div>
      ...
      <div className="flex overflow-x-auto snap-x snap-mandatory gap-4 px-4 pb-4 no-scrollbar">
        {items.map((item) => (
          <div
            ...
            className="snap-center shrink-0 w-[85vw] glass-card rounded-2xl p-5 flex flex-col justify-between border border-slate-800 shadow-xl active:scale-95 transition-transform"
          >
            <div>
            ...
            <div className="pt-3 border-t border-slate-800/80 flex items-center justify-between text-xs">
              <span className="text-slate-400 font-mono text-[11px]">터치하여 상세 보기</span>
              <div className="flex items-center gap-1 font-semibold text-white" style={{ color: colorPreset.primary }}>
                <span>미리보기</span>
